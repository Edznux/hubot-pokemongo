...
var newScan = function(){
	...
	if(tmpList.length == 0){
		// no address set at all
		return;
	}
	addr = tmpList[SCAN_COUNT]
	url = SCAN_URL+addr;

	console.log("Scan request for :", addr, "at", new Date(), "scan count :", SCAN_COUNT, "over",tmpList.length)
	...
	if(user.notif){
		...
		if(user.timerLeft > 0){
			...
			getPokemonByAddr(addr, function(err,data){
				if(err){
					if(err.indexOf("Checking your browser before accessing") !== -1){
						console.error("Cloudflare enabled on pokevision ...")
					}
					if(err.indexOf("Maintenance") !== -1){
						console.error("Maintenance on pokevision ...")
					}
					return;
				...
				for(var k = 0; k < data.pokemon.length; k++){
					...
					if(isInRange(addr, pokemonCoord, range)){
						...
						msg += getDialogs("in_range", user.locale) + " ";
						msg += getDialogs("despawn_in", user.locale) + " ";
						...
						if(user.map){
							locations+="&markers=icon:http://inva.fr/hubot-pogo/small_"+data.pokemon[k].pokemonId+".png%7C"+data.pokemon[k].latitude + ","+ data.pokemon[k].longitude;
						}
						msg+="\n";
					}
				}
				...
				if(user.map && msg !== ""){
					if(!user.mapsize){
						user.mapsize = DEFAULT_MAP_SIZE;
					}
					msg+="http://maps.google.com/maps/api/staticmap?size="+user.mapsize+"&maptype=roadmap&format=png"+locations;
				}

				console.log("++++++++++++++++++++++"+user.user+"++++++++++++++++++");

				if(msg == "" && user.debug){
					msg=getDialogs("no_pokemon_found",user.locale) + " " + range + "m"
				}
				console.log("send message :", msg, "to id :",user.id,"name :",user.user,")");
...
var getDialogs = function(item, locale){
	if(!locale){
		locale = DEFAULT_LOCALE;
	}
