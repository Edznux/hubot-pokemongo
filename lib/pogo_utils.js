						...
						getPokemonByAddr(addr, function(err,data){
							...
							for(var k = 0; k < data.pokemon.length; k++){
								...
								if(isInRange(addr, pokemonCoord, range)){
									msg += getPokemonById([data.pokemon[k].pokemonId], locale)+" en range, Despawn dans :"+((data.pokemon[k].expiration_time- Date.now()/1000)/60).toFixed(2)+"min (lon :"+data.pokemon[k].longitude+ " lat :"+ data.pokemon[k].latitude +")\n";
								}
							}
							console.log("++++++++++++++++++++++"+list[i].user+"++++++++++++++++++");
