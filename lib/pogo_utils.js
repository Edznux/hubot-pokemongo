...
var interval = 60*1*1000; // 1 min interval in ms
	...
	https.get(url,function(res){
		...
		res.on('end', function(){
			try{
				...
				console.error(data.substring(100));
...
var alertNearby = function(){
	console.log("ALERTING NEARBY POKEMON")
				...
				if(list[i].notif){
					getPokemonByAddr(addr, function(err,data){
						...
						for(var k = 0; k < data.pokemon.length; k++){
							...
							range = list[i].range == null ? 100 : list[i].range;
							if(isInRange(addr, pokemonCoord, range)){
								console.log("========================",POKEMON_LIST[data.pokemon[k].pokemonId]);
								msg += POKEMON_LIST[data.pokemon[k].pokemonId]+" en range, Despawn dans :"+((data.pokemon[k].expiration_time- Date.now()/1000)/60).toFixed(2)+"min (lon :"+data.pokemon[k].longitude+ " lat :"+ data.pokemon[k].latitude +")\n";
							}
						}
						if(msg == "" && list[i].debug){
