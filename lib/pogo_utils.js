	...
	if(user.notif){
		...
		if(user.timerLeft > 0){
			...
			getPokemonByAddr(addr, function(err,data){
				console.log("getPokemonByAddr : ", user);
				if(err){
				...
				for(var k = 0; k < data.pokemon.length; k++){
					...
					if(isInRange(addr, pokemonCoord, range)){
						...
						if(user.map){
							locations+="&markers="+data.pokemon[k].latitude + ","+ data.pokemon[k].longitude;
						}
						msg+="\n";
					}
				}
				if(user.map){
					msg+="http://maps.google.com/maps/api/staticmap?size=200x200&maptype=roadmap&format=png"+locations;
				}
