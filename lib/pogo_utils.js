...
var LOCALES = [];
	LOCALES["fr"] = require("../locales/fr.json");
	LOCALES["de"] = require("../locales/de.json");
			...
			getPokemonByAddr(addr, function(err,data){
				...
				for(var k = 0; k < data.pokemon.length; k++){
					...
					if(isInRange(addr, pokemonCoord, range)){
						msg += getPokemonById([data.pokemon[k].pokemonId], locale)+" in range. Despawn in :"+((data.pokemon[k].expiration_time- Date.now()/1000)/60).toFixed(2)+"min (lon :"+data.pokemon[k].longitude+ " lat :"+ data.pokemon[k].latitude +")\n";
					}
				}
				console.log("++++++++++++++++++++++"+user.user+"++++++++++++++++++");

				if(msg == "" && user.debug){
					return robot.messageRoom(user.user,"DEBUG : no pokemon found in "+range+"m range");
				}
