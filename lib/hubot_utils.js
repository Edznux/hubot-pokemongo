...
var showPreferencesByUser = function (user, callback){
	...
	for(var i=0; i < list.length; i++){
		if(list[i].user === user){
			msg += "Preferences for user : "+list[i].user + "\n";
			msg += "\t - Debug : "+ (list[i].debug ? "Enabled" : "Disabled")+"\n";
			msg += "\t - Notification : "+ (list[i].notif ? "Enabled" : "Disabled")+"\n";
			msg += "\t - Range (+/-) : "+ list[i].range+"\n";
			msg += "\t - Notification interval : "+ list[i].timer + " ( "+list[i].timerLeft+" minute(s) left)"+"\n";
