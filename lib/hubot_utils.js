...
var addAddrToUser = function (lat, long, user, callback){
	...
	list.push({"username":user.name,"id":user.id, "addr":[addr], "notif":DEFAULT_NOTIF, "debug":DEFAULT_DEBUG, "timer":DEFAULT_TIMER, "timerLeft":DEFAULT_TIMER, "range":DEFAULT_RANGE, "locale":DEFAULT_LOCALE});
...
var listAddrFromUser = function(user, callback){
	searchList(user, function(err, usr){
		...
		if(usr.addr.length === 0){
			callback(pu.getDialogs("no_addr",usr.locale), null);
		}
...
var enableMapToUser = function (user, callback){
	...
	searchList(user, function(err, usr){
		...
		usr.map = true;
		return callback(null, user+" enabled map !");
	});

};

module.exports.enableMapToUser = enableMapToUser;

var disableMapToUser = function (user, callback){
	...
	searchList(user, function(err, usr){
		...
		usr.map = false;
		return callback(null, user+" disabled map !");
	});

};

module.exports.disableMapToUser = disableMapToUser;
...
var showPreferencesByUser = function (user, callback){
	...
	searchList(user, function(err, usr){
		...
		msg += "\t - Map : "+ (usr.map ? "Enabled" : "Disabled")+"\n";
		msg += "\t - Range (+/-) : "+ usr.range+"\n";
		msg += "\t - Map size : "+ usr.mapsize+"\n";
...
function searchList(user, callback){
	...
	for(var i=0; i < list.length; i++){
		if(list[i].user === user){
			if(!list[i].id){
				return callback("Please update your profile by doing `pogo add 0 0` then `pogo rm 0 0`. This is needed after updating hubot-pokemongo from 1.1.1 and in rare case some data corruption", null);
			}
...
var setMapSizeToUser = function (x, y, user, callback){

	if(x == undefined || y == undefined ){
		return callback("Syntax error in parameters",null)
	}
	
	if(isNaN(x) || isNaN(y)){
		return callback("Syntax error in parameters (Not a number)",null)
	}

	if(parseInt(x) <= 0 || parseInt(y) <= 0){
		return callback("Syntax error in parameters (Must be positive integer)",null)
	}
	...
	searchList(user, function(err, usr){
		...
		return callback(null, "Map size set to : "+ usr.mapsize + " for " + user);
	});

};

module.exports.setMapSizeToUser = setMapSizeToUser;
